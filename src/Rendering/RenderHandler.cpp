...
bool RenderHandler::registerRender(std::shared_ptr<GameObjects::Renderable> registrant) {
    renderables.push_back(registrant);
    return true;
}

bool RenderHandler::unregisterRender(std::shared_ptr<GameObjects::Renderable> registrant) {
    int counter = 0;
...
void RenderHandler::init(uint32_t _width, uint32_t _height, HWND _renderWindow, std::shared_ptr<GameObjects::Camera> _activeCamera) {
...
void RenderHandler::loadRendererAssets() {
    ...
    // Create the pipeline state, which includes compiling and loading shaders.
    {
        ...
        ThrowIfFailed(D3DCompileFromFile(L"D:\\personal one drive\\OneDrive\\Documents\\engineSandbox\\data\\shaders\\pixel\\pixelShader.hlsl", nullptr, nullptr, "PSMain", "ps_5_0", compileFlags, 0, &pixelShader, nullptr));
        ThrowIfFailed(D3DCompileFromFile(L"D:\\personal one drive\\OneDrive\\Documents\\engineSandbox\\data\\shaders\\vertex\\vertexShader.hlsl", nullptr, nullptr, "VSMain", "vs_5_0", compileFlags, 0, &vertexShader, nullptr));
...
void RenderHandler::populateCommandList() {
    ...
    for (const std::shared_ptr<GameObjects::Renderable>& item : renderables) {
...
std::shared_ptr<SMeshDataGPU> RenderHandler::createGpuMesh(const Assets::SMeshDataCPU& meshData) {
    ...
    auto result = std::make_shared<SMeshDataGPU>();
    ...
    // ------------------------------------------------------------
    {
        const UINT vertexBufferSize =
            static_cast<UINT>(meshData.verts.size() * sizeof(SVertex));
    ...
    // ------------------------------------------------------------
    {
        const UINT indexBufferSize =
            static_cast<UINT>(meshData.indices.size() * sizeof(std::uint32_t));
...
void RenderHandler::createRenderableConstantBuffer(GameObjects::Renderable& item) {
    ...
    const UINT constantBufferSize =
        (sizeof(SRenderableConstants) + 255) & ~255;
...
void RenderHandler::updateRenderableConstantBuffer(GameObjects::Renderable& item) {
    if (item.mappedConstantBuffer == nullptr) {
        throw std::runtime_error("Renderable constant buffer is not mapped.");
    }

    SRenderableConstants constants = {};
