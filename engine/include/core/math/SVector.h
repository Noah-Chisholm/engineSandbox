...
inline math::fVector operator*(float val, const math::fVector& vec) {
    return vec * val;
}
...
template<>
struct std::formatter<math::fVector, char> {
    constexpr auto parse(std::format_parse_context& ctx) {
        return ctx.begin();
    }

    auto format(const math::fVector& vec, std::format_context& ctx) const {
